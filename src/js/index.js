...
async function main() {
  ...
  // Check for CSV-only mode
  const csvOnly = process.env.CSV_ONLY?.toLowerCase() === "true" || 
                  process.env.CSV_ONLY === "1" || 
                  process.env.CSV_ONLY?.toLowerCase() === "yes";
  
  if (csvOnly) {
    console.log("\n✅ CSV_ONLY mode is enabled.");
    console.log("This script will not run. CSV files have been generated by the Python script.");
    console.log("To create playlists and manage Watch Later, set CSV_ONLY=false in .env");
    return;
  }
